...
@section('content')
    <div class="p-6 lg:p-8">
        ...
        @php
            $gradeColors = ['a' => '#6ea89e', 'b' => '#a6d2c8', 'c' => '#fbbf24', 'd' => '#f97316', 'e' => '#dc2626', 'unknown' => '#9ca3af'];
            $gradeLabels = ['a' => 'Sangat Sesuai', 'b' => 'Sesuai', 'c' => 'Sedang', 'd' => 'Kurang', 'e' => 'Buruk'];
            $ng = strtolower($product['nutrition_grades'] ?? 'unknown');
            ...
            if (!empty($product['serving_size'])) {
                preg_match('/(\d+)/', $product['serving_size'], $m);

                if (isset($m[1])) {
                    $servingFactor = $m[1] / 100;
                }
            }
        @endphp
        ...
        <div class="flex flex-col md:flex-row gap-8 mb-8">
            ...
            <div class="md:w-72 flex-shrink-0">
                <div class="bg-gray-200 rounded-xl overflow-hidden aspect-square">
                    ...
                    @else
                        <div class="flex items-center justify-center h-full text-gray-400 font-semibold text-xl">Gambar Produk
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex-1">
                ...
                <div class="flex flex-wrap gap-3">
                    <div class="px-4 py-2 rounded-lg text-white font-semibold text-sm" style="background-color: {{ $gc }}">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-600 text-white">
                        <th class="text-left p-4 text-sm font-semibold border-r border-black/10">Informasi<br>nilai gizi
                        </th>
                        <th class="text-left p-4 text-sm font-semibold border-r border-black/10">As sold untuk<br>100 g /
                            100 ml</th>
                        <th class="text-left p-4 text-sm font-semibold">As sold per serving<br>({{ $serving }}) (packaging)
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    @php
                        $rows = [
                            ['label' => 'Energi', 'key100' => $n['energy'] ?? null, 'unit' => 'kcal', 'indent' => false],
                            ['label' => 'Lemak', 'key100' => $n['fat'] ?? null, 'unit' => 'g', 'indent' => false],
                            ['label' => 'Lemak jenuh', 'key100' => $n['saturated-fat'] ?? null, 'unit' => 'g', 'indent' => true],
                            ['label' => 'Karbohidrat', 'key100' => $n['carbohydrates'] ?? null, 'unit' => 'g', 'indent' => false],
                            ['label' => 'Gula', 'key100' => $n['sugars'] ?? null, 'unit' => 'g', 'indent' => true],
                            ['label' => 'Serat Pangan', 'key100' => $n['fiber'] ?? null, 'unit' => 'g', 'indent' => false],
                            ['label' => 'Protein', 'key100' => $n['proteins'] ?? null, 'unit' => 'g', 'indent' => false],
                            ['label' => 'Garam', 'key100' => $n['salt'] ?? null, 'unit' => 'g', 'indent' => false],
                            ['label' => 'Natrium', 'key100' => $n['sodium'] ?? null, 'unit' => 'g', 'indent' => false],
                            ['label' => 'Fruits vegetables legumes', 'key100' => $n['fruits'] ?? null, 'unit' => '%', 'indent' => false],
                        ];
                    @endphp
                    @foreach($rows as $i => $row)
                        <tr class="{{ $i % 2 === 0 ? 'bg-gray-600/80' : 'bg-gray-500/70' }} text-white">
                            ...
                            <td class="p-4 text-sm border-r border-black/10">
                                @if($row['key100'] !== null)
                                    @if($row['unit'] === 'kcal')
                                        {{ number_format($row['key100'], 0) }} kcal
                                    @elseif($row['unit'] === '%')
                            ...
                            <td class="p-4 text-sm">
                                @if($row['key100'] !== null)
                                    ...
                                    @elseif($row['unit'] === '%')
                                        ?
                                    @else
                                        {{ rtrim(rtrim(number_format($row['key100'] * $servingFactor, 5, '.', ''), '0'), '.') }}
                                        {{ $row['unit'] }}
                                    @endif
                                @else
                                    ?
